...
function getResource(request: NextRequest) {
  ...
  return (searchParams.get('resource') || 'progress').toLowerCase()
}

export async function GET(request: NextRequest) {
  const resource = getResource(request)
  if (resource === 'progress') return handleProgressGet(request)
  return NextResponse.json({ error: 'Method not allowed' }, { status: 405 })
}

export async function POST(request: NextRequest) {
  const resource = getResource(request)
  if (resource === 'progress') return handleProgressPost(request)
  if (resource === 'adaptive') return handleAdaptivePost(request)
  return NextResponse.json({ error: 'Unknown progress resource' }, { status: 400 })
}

async function handleProgressGet(request: NextRequest) {
...
async function handleProgressPost(request: NextRequest) {
  const ip = getIp(request)
  if (isRateLimited(`${ip}:progress`, 60, 60_000)) {
    return NextResponse.json({ error: 'Too many requests' }, { status: 429 })
  }
...
async function handleAdaptivePost(request: NextRequest) {
  const ip = getIp(request)
