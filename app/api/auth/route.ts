...
const ADMIN_SITE_URL = process.env.SITE_URL || process.env.APP_URL || 'https://synaptiq.co.uk'
...
function weeklyEmailHtml(name: string, stats: { questions: number; accuracy: number; xp: number; streak: number }) {
  ...
  return `<div style="font-family:'DM Sans',sans-serif;max-width:600px;margin:0 auto;background:#0D0F18;color:#F0EEF8;border-radius:16px;overflow:hidden">
    ...
    <div style="padding:2rem">
      ...
      <a href="${ADMIN_SITE_URL}/dashboard" style="display:inline-block;background:#4F8CFF;color:#fff;padding:.875rem 2rem;border-radius:10px;font-weight:700;text-decoration:none;margin-top:1rem">Keep Going</a>
    </div>
    <div style="padding:1rem 2rem;border-top:1px solid rgba(255,255,255,0.07);font-size:.8rem;color:#6B7394;text-align:center">
      Synaptiq &middot; <a href="${ADMIN_SITE_URL}/privacy" style="color:#6B7394">Privacy</a>
    </div>
  </div>`
}
...
export async function GET(request: NextRequest) {
  // Admin dashboard GET requests (stats, users listing)
...
export async function POST(request: NextRequest) {
  // Admin dashboard POST requests (actions with x-admin-key header)
  if (request.headers.get('x-admin-key')) {
    return handleAdmin(request)
  }
