...
const FROM_ADDRESS = 'Synapnode <hello@synaptiq.co.uk>'
...
function buildTemplates(name: string, stats: Record<string, unknown>, siteUrl: string) {
  ...
  const foot = `<div style="padding:1rem 2rem;border-top:1px solid rgba(255,255,255,0.07);font-size:.8rem;color:#6B7394;text-align:center">Synapnode &middot; <a href="${siteUrl}/privacy" style="color:#6B7394">Privacy</a> &middot; <a href="${siteUrl}/terms" style="color:#6B7394">Terms</a></div></div>`
  ...
  return {
    ...
    payment_confirmed: {
      subject: 'Your Synapnode subscription is active!',
      html: wrap(
        hdr("You're in!"),
        `<p>Your <strong>${stats?.plan === 'homeschool' ? 'Homeschool' : 'Student'} Plan</strong> is now active. You have full access to all Synapnode features.</p>${cta('Start Learning')}`,
      ),
    },
    payment_failed: {
      subject: 'Synapnode — Payment failed',
      html: wrap(
        '',
    ...
    parent_report: {
      subject: `${name}'s A-Level Maths progress report — Synapnode`,
      html: wrap(
        hdr('Parent Progress Report', `Synapnode A-Level Maths · ${dateStr}`),
    ...
    password_reset: {
      subject: 'Reset your Synapnode password',
      html: wrap(
        hdr('Password Reset', 'Synapnode — A-Level Maths AI Tutor'),
        `<h2 style="color:#C9A84C">Hi ${name},</h2><p>We received a request to reset your Synapnode password. Use the link sent to your inbox — it expires in <strong>1 hour</strong>.</p><p style="color:#6B7394;font-size:.875rem">If you didn't request this, you can safely ignore this email.</p>${cta('Back to Synapnode')}`,
      ),
    },
    goodbye: {
      subject: 'Your Synapnode account has been deleted',
      html: `<div style="font-family:'DM Sans',sans-serif;max-width:600px;margin:0 auto;background:#0D0F18;color:#F0EEF8;border-radius:16px;overflow:hidden"><div style="background:linear-gradient(135deg,#3a3a4a,#1e1e2e);padding:2rem;text-align:center"><h1 style="font-size:1.8rem;margin:0;color:#F0EEF8">Account Deleted</h1><p style="opacity:.6;margin:.5rem 0 0;color:#F0EEF8">We'll miss you, ${name}</p></div><div style="padding:2rem"><p>Your account and all data have been permanently deleted from our servers.</p><div style="background:#181C2A;border-radius:10px;padding:1.25rem;margin:1.5rem 0"><p style="margin:0 0 .5rem;font-weight:700;color:#C9A84C">Changed your mind?</p><p style="margin:0;font-size:.875rem;color:#6B7394">You can create a new Synapnode account at any time.</p></div><a href="${siteUrl}" style="display:inline-block;background:#C9A84C;color:#08090E;padding:.875rem 2rem;border-radius:10px;font-weight:700;text-decoration:none;margin-top:.5rem">Create New Account</a></div><div style="padding:1rem 2rem;border-top:1px solid rgba(255,255,255,0.07);font-size:.8rem;color:#6B7394;text-align:center">Synapnode &middot; <a href="${siteUrl}/privacy" style="color:#6B7394">Privacy</a> &middot; <a href="${siteUrl}/terms" style="color:#6B7394">Terms</a></div></div>`,
    },
  }
}
