...
describe('image content validation', () => {
  function imageMsg(media_type, dataLength = 100) {
    return [{
      role: 'user',
      content: [
        { type: 'text', text: 'What is this?' },
        { type: 'image', source: { type: 'base64', media_type, data: 'A'.repeat(dataLength) } },
      ],
    }];
  }

  it('returns 400 for an unsupported image media_type', async () => {
    const r = res();
    await handler(req({ messages: imageMsg('image/bmp') }), r);
    expect(r.statusCode).toBe(400);
    expect(r.body.error).toMatch(/unsupported image type/i);
  });

  it('accepts jpeg, png, gif, and webp media types', async () => {
    for (const type of ['image/jpeg', 'image/png', 'image/gif', 'image/webp']) {
      ...
      const r = res();
      await handler(req({ messages: imageMsg(type) }), r);
      expect(r.statusCode).toBe(200);
    }
  });

  it('returns 400 when an image exceeds 5 MB', async () => {
    // base64 chars × 0.75 > 5 MB  →  need more than Math.ceil(5 MB / 0.75) chars
    const oversizedLength = Math.ceil((5 * 1024 * 1024) / 0.75) + 1;
    const r = res();
    await handler(req({ messages: imageMsg('image/jpeg', oversizedLength) }), r);
    expect(r.statusCode).toBe(400);
    expect(r.body.error).toMatch(/image too large/i);
  });

  it('returns 400 when more than 5 images are included', async () => {
    const content = [{ type: 'text', text: 'Many images' }];
    for (let i = 0; i < 6; i++) {
      content.push({ type: 'image', source: { type: 'base64', media_type: 'image/jpeg', data: 'A'.repeat(100) } });
    }
    const r = res();
    await handler(req({ messages: [{ role: 'user', content }] }), r);
    expect(r.statusCode).toBe(400);
    expect(r.body.error).toMatch(/too many images/i);
  });

  it('accepts a message with exactly 5 valid images', async () => {
    const content = [{ type: 'text', text: 'Five images' }];
    for (let i = 0; i < 5; i++) {
      content.push({ type: 'image', source: { type: 'base64', media_type: 'image/png', data: 'A'.repeat(100) } });
    }
    global.fetch = vi.fn().mockResolvedValueOnce({
      status: 200,
      json: () => Promise.resolve({ content: [{ text: 'ok' }] }),
